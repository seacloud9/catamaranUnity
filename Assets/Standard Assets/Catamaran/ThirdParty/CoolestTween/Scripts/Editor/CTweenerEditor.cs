	...
	public class CTweenerEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			if (myTarget is CTweenPosition) {
				...
				Transform tr = (Transform)EditorGUILayout.ObjectField (null, typeof(Transform), true);
				if (tr != null)
			...
			if (GUILayout.Button ("Switch", GUILayout.Width (64))) {
				if (myTarget is CTweenPosition) {
					...
					Color temp;
					if (myTarget.GetComponent<Renderer> () is SpriteRenderer) {
						temp = (myTarget.GetComponent<Renderer> () as SpriteRenderer).color;
						(myTarget.GetComponent<Renderer> () as SpriteRenderer).color = (myTarget as CTweenColor).toColor;
					} else {
						temp = myTarget.GetComponent<Renderer> ().sharedMaterial.color;
						myTarget.GetComponent<Renderer> ().sharedMaterial.color = (myTarget as CTweenColor).toColor;
					}
